---
name: hallmark
description: "Anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build a new app or landing page, wants to redesign something, invokes Hallmark by name, or uses audit/redesign/study. Run after brainstorming approval; does not replace responsive-design or scss-best-practices for implementation details."
version: 1.0.0
---
...
Run **after brainstorming approval** (Superpowers design gate). Hallmark owns visual structure and anti-slop design — not responsive breakpoints, container queries, or SCSS architecture. Defer those to **responsive-design** and **scss-best-practices** during implementation.

**Feature work in an existing app:** When building incrementally inside `/new-feature` or similar pipelines, Hallmark sets visual voice; **`design-system`** owns where tokens and shared components grow — read `~/.agents/skills/design-system/SKILL.md` during planning and implementation. Do not let feature work fork one-off styles that bypass `tokens.css` or `design.md`.
...
| Skill | Role |
|-------|------|
| **`design-system`** | Grow shared tokens and components during `/new-feature` work — defers visual voice to Hallmark |
| **`responsive-design`** | Breakpoints, container queries |
| **`scss-best-practices`** | SASS module architecture |
| **`radix-ui-design-system`** | Headless primitive a11y (forms defer field wiring to `react-hook-form-zod`) |
