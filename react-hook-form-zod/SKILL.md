...
description: "Type-safe React forms with React Hook Form and Zod validation. Use for form schemas, field arrays, multi-step forms, resolver issues, nested field problems, or implementing/fixing form validation. edge-cases finds validation logic gaps; this skill owns schemas, resolvers, and field wiring."
...
| This skill | Defer to |
|------------|----------|
| Zod schemas, `zodResolver`, `register`/`Controller`, field arrays, multi-step wiring | — |
| Form validation logic gaps found in audit (empty submit, missing guards, schema mismatch) | `edge-cases` finds them; implement fixes here |
| Auth on submit, server-side sanitization, injection | `security-audit` |
| Radix primitive a11y (Dialog, Select, Popover focus/ARIA) | `radix-ui-design-system` |
...
Form-field patterns (labels, `aria-invalid`, error announcements) live here. For Radix-based components (Select, Dialog, Checkbox, etc.), defer to **radix-ui-design-system** accessibility sections.
