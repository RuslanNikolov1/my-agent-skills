...
Set these on every HTML response. Test at securityheaders.com or Mozilla Observatory.
...
Add `preload` to HSTS only after every subdomain is HTTPS permanently.
...
```
# public/security.txt → served at /.well-known/security.txt
...
- **Custom 404 / 500:** return the **correct** status code (a "not found" page must be `404`, not `200`; see soft-404 gotchas in `SKILL.md`). Explain the problem in plain language and offer a way forward; never leak stack traces.
...
```typescript
// Route Handler or middleware — maintenance window
